...
app = FastAPI(title='台账查找后端', version='2.3.0')
...
def restore_from_backup(db: Session, zf: zipfile.ZipFile, manifest: dict):
    ...
    # 0) 先按备份同步柜子数量、门型、名称、颜色（完整还原柜体配置）
    apply_config(db, [{
    ...
    # 建立备份 cabinetId -> 当前柜序号的映射（正常情况下两者一致，此处做容错）
    cab_id_map = {}
    ...
    # 3) 还原文件（cabinetId/shelf/slot 与备份中的台账位置严格对应）
    for fm in (manifest.get('files') or []):
        try:
            content = zf.read(fm['storedName'])
        except KeyError:
            continue
        # 柜号映射：优先使用备份中的 cabinetId，经映射后写入；越界则跳过避免错位
        ...
        if not (0 <= f_shelf < SHELF_COUNT and 0 <= f_slot < MAX_BOXES_PER_SHELF):
            continue
        # 确保对应台账格子存在，文件挂到正确的柜子/层/位置
    ...
    # 4) 还原用户（尽量保持原 id，避免登录态失效；内置管理员始终为最高权限，不可被降级）
...
def upload_file(request: Request, body: UploadIn, token: str = Depends(get_token), db: Session = Depends(get_db)):
    ...
    # 注意：上传文件不再同步修改台账名称，台账名称保持后台管理/导入时设定的内容
    f = File(
...
def change_password(body: ChangePasswordIn, token: str = Depends(get_token), db: Session = Depends(get_db)):
    user = require_user(token, db)
    if not user:
        return err(401, '未登录')
