...
- **双重名称**：每个台账显示「文档编号（横向）+ 台账名称（竖向）」，编号由后台维护，留空时自动按层内序号显示（01、02 …），搜索支持编号与名称。
- **搜索定位**：搜索台账名称或编号，自动飞行定位并高亮。
- **详情查看**：点击任一台账放大突出（不变），放大时顶部单独出现「查看详情」按钮，点击按钮查看所在位置、编号、名称及该台账下统一上传的文件（图片 / PDF / 文本）；再次点击该台账则收回（台账恢复原大小，镜头保持在放大界面）。文件「下载」在 APK 内直接保存到设备下载目录。
- **后台管理（网页）**：浏览器打开 `http://<服务器IP>:10600/admin` 登录后可管理：
  - 目录：每个柜子每层台账数量最多 **40** 个，名称可改、编号可填，数量可增删。
  - 导入 / 导出：下载 CSV 模板（柜号,层号,序号,文档编号,台账名称），用 Excel 填好编号与名称后上传批量导入；支持「导出 CSV」下载当前目录（含编号）。
...
## 内网安全（v3.0 起，当前 v4.0.1）

针对“防止通过内网偷取资料”，后端做了以下加固（Python 版与 Node 版行为一致，当前 v4.1.0）：
